...
Charts show Month To Date, Quarter To Date, and Year To Date data when available

Data source is PC NetBoB Daily report

```{r setup, include=FALSE}

#start.date1 <- as.Date("2017-05-01")
#end.date1 <- as.Date("2017-05-09")


knitr::opts_chunk$set(echo = TRUE)

## Not generating even one plot at the moment
## Most likely schcap.match.R is deleting the rows due to incompatibility #with input
...
nb.horiz.bars.owner <- function(fixed.yy, owner.initname, time.range.name){
  ...
  fixed.agg <- if(nrow(fixed.pgrouphh) < 1){
    data.frame("SCHOOL.NAME" = 0, "PROGRAM" = 0, "TRANSACTION.YEAR" = 0, "NETBOB" = 0)
  } else {
    aggregate(NETBOB ~ SCHOOL.NAME + PROGRAM + TRANSACTION.YEAR, fixed.pgrouphh, sum)
  }
...
new.mtd.fix <- mtd.prep(new.mtd, class.data, "2016-01-01", "2017-05-31")
...
owner.len <- length(owner.vec.clean)
...
for(o in 1:owner.len){
  for(p in 1:3){
    ptd.yy.fix <- yy.periodtodate(ly.ytd.fix, period.start.vec[p])
    cy <- ptd.yy.fix[ptd.yy.fix$TRANSACTION.YEAR %in% 2017,]
    py <- ptd.yy.fix[ptd.yy.fix$TRANSACTION.YEAR %in% 2016,]
    yy.matched <- schcap.match(cy, py)
    bars <- nb.horiz.bars.owner(yy.matched, owner.vec.clean[o], period.names.vec[p])
  }
}
